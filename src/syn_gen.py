...
def syn_gen(
    ...
    # ------------------------------------------------------------------
    n_sites, n_fit, n_leads, n_ens = hcst_fit.shape         # (# sites, # HEFS dates, # leads, # ensemble members)
    n_gen = np.shape(obs_fwd_gen)[1]
    ...
    obs_forward_gen = obs_fwd_gen               # (n_sites, n_gen, leads)
    # obs_forward for simulation dates (intersection with ixx_gen)
    obs_forward_fit = obs_fwd_fit               # (n_sites, n_fit, leads)
    ...
    # ------------------------------------------------------------------
    gen_knn_data = obs_forward_gen[keysite_idx,:,:]
    fit_knn_data = obs_forward_fit[keysite_idx,:,:]
    ...
    for j in range(n_sites):
        ...
        for k in range(n_leads):
            ...
            #relax scaling threshold proportional to any obs in obs_fwd row that is greater than the max fit period obs
            ext_scale_mat = gen_scale / np.max(obs_forward_fit[j,:,:])
            ...
            #calculate activated region of threshold for each row based on the obs value w/relaxed scaling where needed
